...
>- Average may be interesting on its own, \ldots \pause but often masks assumption of _homogeneous_ effects
...
W <- mp_train |> select(is_winner) |> unlist() |> as.numeric() 
...
cf_preds <- predict(cf_out, X_test)$predictions
...
ggplot(df_cf, aes(x = sort(age), y = sort(cf_te))) + 
  geom_point() + 
