...
>- Introduction to several approaches
>- A survey 
>- Examples: 
>    - low-dimensional
>    - available data
>    - (experimental)
>    - estimation/interpretation practice
>- Lab exercises last hour each day
>- Timing should $\approx$ work out \ldots
>- Materials here:  
https://github.com/ryantmoore/new-directions-berlin
...
1 & Yes & \only<3->{Yes} & \only<7->{\alert{(Yes)}} &  Yes \\
2 & Yes & \only<4->{Yes} & \only<7->{\alert{(No)}} &  Yes \\
3 & No & \only<7->{\alert{(Yes)}} & \only<5->{No} &  No \\
4 & No &\only<7->{\alert{(No)}} & \only<6->{No} &  No
\end{tabular}
\end{center}
...
>- Units indexed by $i$
>- Treatment $T_i$ or $D_i$ or $Z_i$ or $W_i$
...
Avg pred treatment effect in test sample:
...
A doubly-robust ATE from training sample:
...
df_lasso <- read_csv("../data/01-lasso.csv")
...
## The Double LASSO for Treatment Effects: Example
...
df_social |> select(-yearofbirth) |> head()
```
...
```{r}
rlasso_out <- rlassoATE(
  primary2006 ~ age + is_male + primary2004 + hhsize + 
    is_neighbors | age + is_male + primary2004 + hhsize, 
  data = df_social)
```
...
```{r}
X <- as.matrix(df_social[, c("age", "is_male", "primary2004", 
                             "hhsize", "is_neighbors")])
